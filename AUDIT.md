...
- ✅ **対応済み**: `.gitignore:21` の `CLAUDE.md` 除外を解除し、Git追跡対象に変更。内容を精査した結果、機密情報・ローカル絶対パス・NAS/IP等は一切含まれておらず、公開リポジトリにそのままコミットして問題ないと判断。`.gitignore` は代わりに（将来ローカル限定の情報が必要になった場合のための）`CLAUDE.local.md` を除外するよう変更済み。
- ✅ **対応済み**: CLAUDE.mdにテスト実行手順（`PYTHONPATH=. pytest tests/ -v`、CI準拠）を追記。
- 未対応: 感情辞書ダウンロード失敗時の挙動明記、requirements.txtのビルド専用パッケージ分離、`src/core/`配下のモジュール構成の追記など、その他の改善提案は今回のスコープ外（下記「改善提案」参照、実装は未着手）。
