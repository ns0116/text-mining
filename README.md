...
本プロジェクトで使用されている分析アルゴリズム（TF-IDF、感情分析、共起ネットワーク、対応分析）の解説や、テキストマイニングの基本プロセスについては、以下の資料を参照してください。

* **[技術解説・仕様書](file://docs/technical_guide.md)**
  * TF-IDFの数理モデル、否定表現を考慮した感情判定の仕組み、Jaccard係数に基づく共起算出、対応分析 (SVD) などの詳細ロジックを解説しています。
* **[解説インフォグラフィック](file://docs/text-mining-infographic.html)**
  * テキストマイニングのステップやアルゴリズムを視覚的かつ直感的に解説したHTMLファイルです。ブラウザで開くことで、勉強会や共有用リソースとしてそのまま利用できます。
